...
from opendbc.car.volvo.values import VolvoFlags, DBC, CarControllerParams as CCP
    ...
    if (self.frame % 2 == 0):
      ...
      if enabled and CS.out.vEgo > self.CP.minSteerSpeed:
        ...
        if self.angle_request_prev * self.SteerCommand.angle_request > 0. and abs(self.SteerCommand.angle_request) > abs(self.angle_request_prev):
          angle_rate_lim = float(np.interp(CS.out.vEgo, CCP.ANGLE_DELTA_BP, CCP.ANGLE_DELTA_V))
        else:
          angle_rate_lim = float(np.interp(CS.out.vEgo, CCP.ANGLE_DELTA_BP, CCP.ANGLE_DELTA_VU))

        self.SteerCommand.angle_request = float(np.clip(self.SteerCommand.angle_request, self.angle_request_prev - angle_rate_lim, self.angle_request_prev + angle_rate_lim))
        ...
        self.SteerCommand.trqlim = 0
        if self.CP.flags & VolvoFlags.C1:
          self.SteerCommand.angle_request = float(np.clip(CS.out.steeringAngleDeg, -359.95, 359.90))  # Cap values at max min values (Cap 2 steps from max min). Max=359.99445, Min=-360.0384
        else:
      ...
      if self.CP.flags & VolvoFlags.C1:
        ...
        if (self.trq_fifo.count(0) >= CCP.N_ZERO_TRQ) and (self.fault_frame == -200):
          self.fault_frame = self.frame+100

        if enabled and (self.frame < self.fault_frame):
          self.SteerCommand.steer_direction = CCP.STEER_NO
      ...
      can_sends.append(volvocan.manipulateServo(self.packer, self.CP, CS))
    
      # send can, add to list.
      can_sends.append(volvocan.create_steering_control(self.packer, self.frame, self.CP, self.SteerCommand, CS.FSMInfo))
    
    
    # Cancel ACC if engaged when OP is not.
    if not enabled and CS.out.cruiseState.enabled:
      can_sends.append(volvocan.cancelACC(self.packer, self.CP, CS))
